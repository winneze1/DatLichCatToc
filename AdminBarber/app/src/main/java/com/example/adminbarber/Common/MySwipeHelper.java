package com.example.adminbarber.Common;
...
import com.example.adminbarber.Interface.MyButtonClickListener;
...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public class MyButton{
        ...
        public void onDraw(Canvas c, RectF rectF, int pos)
        {
            ...
            float x = 0, y =0;
            if (imageResId == 0) //hien text
            {
                x = cWidth/2f - r.width() / 2f - r.left;
                y = cHeight/2f + r.height() - r.bottom;
                c.drawText(text, rectF.left + x, rectF.top + y, p);
            }
            else //neu co hinh anh
            {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            if (dX < 0)
            {
                ...
                translateX = dX * buffer.size() * buttonWidth / itemView.getWidth();
                drawButton(c, itemView, buffer, pos, translateX);
            }
        }
        super.onChildDraw(c, recyclerView, viewHolder, translateX, dY, actionState, isCurrentlyActive);
    }

    private void drawButton(Canvas c, View itemView, List<MyButton> buffer, int pos, float translateX) {
        float right = itemView.getRight();
        float dButtonWidth = -1*translateX / buffer.size();
